...
with open('styleB.css') as f:
    st.markdown(f'<style>{f.read()}</style>', unsafe_allow_html=True)

df = pd.read_csv("./Student.csv")
section_b = df[df['Section'] == 'B']
specified_columns = ['Name', 'Language', 'English', 'Maths', 'Science', 'Social', 'Percentage', 'Result',  'Gender']
new_df = section_b[specified_columns].copy()


st.title("Section B Analysis")
...
def pie(subject):
    pass_students = section_b[(section_b[subject] >= 40)]
    fail_students = section_b[(section_b[subject] < 40)]
...
length_section_B = len(df[df['Section'] == 'B'])
section_B_counts = df[df['Section'] == 'B']['Result'].value_counts()
section_B_gender = df[df['Section'] == 'B']['Gender'].value_counts()

# Calculate the average marks for each subject in Section A
average_marks = []
subjects = [ 'Language', 'English', 'Maths', 'Science', 'Social']
for subject in subjects:
    avg_mark = section_b[subject].mean()
...
with col1:
    ...
    fig = px.pie(values=section_B_counts.values, names=section_B_counts.index, hole=0.5)
    fig.update_traces(text = section_B_counts.index, textposition="outside")
    st.plotly_chart(fig, use_container_width=True)
...
with col2:
    ...
    container.write(f"**Total Number of Students: {length_section_B}**")
    ...
    with col1_1:
        ...
        container2_1.write(f"**{section_B_gender['Male']}**")

    with col1_2:
        container2_2 = st.container(border=True)
        container2_2.write("**Girls**")
        container2_2.write(f"**{section_B_gender['Female']}**")

    with col1_1:
        container2_1 = st.container(border=True)
        container2_1.write("**Total Number of Pass**")
        container2_1.write(f"**{section_B_counts['Pass']}**")

    with col1_2:
        container2_2 = st.container(border=True)
        container2_2.write("**Total Number of Fails**")
        container2_2.write(f"**{section_B_counts['Fail']}**")
...
students_90_above = section_b[section_b['Percentage'] >= 90]
num_students_90_above = len(students_90_above)

students_80_above = section_b[(section_b['Percentage'] >= 80) & (section_b['Percentage'] < 90)]
num_students_80_above = len(students_80_above)

students_70_above = section_b[(section_b['Percentage'] >= 70) & (section_b['Percentage'] < 80)]
num_students_70_above = len(students_70_above)

students_60_above = section_b[(section_b['Percentage'] >= 60) & (section_b['Percentage'] < 70)]
num_students_60_above = len(students_60_above)

students_40_above = section_b[(section_b['Percentage'] >= 40) & (section_b['Percentage'] < 60)]
num_students_40_above = len(students_40_above)

students_40_below = section_b[section_b['Percentage'] < 40]
...
pass_students_section_b = df[(df['Section'] == 'B') & (df['Result'] == 'Pass')]
pass_students_section_b_sorted = pass_students_section_b.sort_values(by='Percentage', ascending=False)
top_3_students = pass_students_section_b_sorted[['Name', 'Percentage']].head(3)

fail_students_section_b = df[(df['Section'] == 'B') & (df['Result'] == 'Fail')]
fail_students_section_b_sorted = fail_students_section_b.sort_values(by='Percentage')
bottom_3_students = fail_students_section_b_sorted[['Name', 'Percentage']].head(3)
